...
% Implementiert einen Harris-Corner-Detektor.
    ...
    % a) Ableitung in x-Richtung (Sobel, ...)
      ...
      % TODO
      f_Sobelx = [-1 -2 -1; 0 0 0;1 2 1];           %% kernel für Sobel in x-Richtung(VL s14)
      Iu = conv2(I,f_Sobelx,'same');
   
    % b) Ableitung in y-Richtung (Sobel, ...)

    
      % TODO
      f_Sobely = [-1 0 1;-2 0 2;-1 0 1];            %% kernel für Sobel in y-Richtung(VL s14)
      Iv=conv2(I,f_Sobely,'same');
    ...
    %    trace(M) = Iu^2+Iv^2    
    std_wert = 0.04;   % Standardwert

    
      % TODO
      R=A.*B-C.^2 - std_wert*((A+B).^2);                %% Formular in VL s27
